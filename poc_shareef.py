            ...
            for i in range( min( nbids, nasks, MAX_LAYERS )):
                ...
                if place_bids and i < nbids:
                    ...
                    if positionSize > 0:
                        ...
                        if (((qty * MAX_LAYERS) / 2 + positionSize > MAX_SKEW) or (ULTRACONSERVATIVE == True and (qty * MAX_LAYERS) / 2 + positionSize > self.maxqty * 2.5 * 5 * 3 * 3)):
                            if self.positions[fut]['size'] > 0:
                                ...
                                try:
                                    ...
                                    abc123 = 1
                                gogo = False
                        if (((qty * MAX_LAYERS) / 2 + positionSize > MAX_SKEW * 2) or (ULTRACONSERVATIVE == True and (qty * MAX_LAYERS) / 2 + positionSize > self.maxqty * 2.5 * 5 * 3 * 3 * 2)):
                            if self.positions[fut]['size'] < 0:
                                print('max skew on buy')
                                try:
                                    oid = bid_ords[ i ][ 'orderId' ]
                ...
                if place_asks and i < nasks:
                    ...
                    if positionSize < 0:
                        ...
                        if (((qty * MAX_LAYERS) / 2 + positionSize * -1 > MAX_SKEW) or (ULTRACONSERVATIVE == True and (qty * MAX_LAYERS) / 2 + positionSize * -1 > self.maxqty * 2.5 * 5 * 3)) and self.positions[fut]['size'] < 0:
                            ...
                            gogo = False
                        if (((qty * MAX_LAYERS) / 2 + positionSize * -1 > MAX_SKEW * 2) or (ULTRACONSERVATIVE == True and (qty * MAX_LAYERS) / 2 + positionSize * -1 > self.maxqty * 2 * 1.25 * 5 * 3)) and self.positions[fut]['size'] > 0:
                            print('max skew on sell')
                            try:
                                oid = ask_ords[ i ][ 'orderId' ]
                                cancel_oids.append( oid )
        ...
        if positionSize is  0 and (skewingpos is 0 and skewingneg is 0):  
            ...
            for p in self.arbmult.keys():
                ...
                if p != 'BTC-PERPETUAL':
                    ...
                    print(self.arbmult)
                    if self.arbmult[p]['arb'] > 1 :
                        self.client.sell(  p, self.maxqty * 2.5 * 5 * 3 * 2 / len(self.futures), mid * 0.98, 'false' )
                    else:
                        self.client.buy(  p, self.maxqty * 2.5 * 5 * 3 * 2 / len(self.futures), mid * 1.02, 'false' )
