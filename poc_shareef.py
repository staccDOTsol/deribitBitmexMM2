            ...
            if self.positionGains[fut] == True:
                ...
                if place_bids:
                    ...
                    if self.positions[fut]['size'] > 0 and self.positionGains[fut] == True:
                        bids    = [ bid0 * riskfac * 1 ** -i for i in range( 1, int(nbids) + 1 ) ]
                    else:
                ...
                if place_asks:
                    ...
                    if self.positions[fut]['size'] < 0 and self.positionGains[fut] == True:
                        ...
                        asks    = [ ask0 * riskfac * 1 ** i for i in range( 1, int(nasks) + 1 ) ]
                    else:   
                ...
                if place_bids:
                    ...
                    bid0            = mid_mkt * math.exp( -MKT_IMPACT )
                    if self.positions[fut]['size'] > 0 and self.positionGains[fut] == True:
                        bids1    = [ bid0 * riskfac * 1 ** -i for i in range( 1, int(nbids) + 1 ) ]
                    else:
                ...
                if place_asks:
                    ...
                    if self.positions[fut]['size'] < 0 and self.positionGains[fut] == True:
                        ...
                        asks1    = [ ask0 * riskfac * 1 ** i for i in range( 1, int(nasks) + 1 ) ]
                    else:   
    ...
    def update_status( self ):
        ...
        if positionSize is  0 and (skewingpos is 0 and skewingneg is 0):  
            ...
            for p in self.arbmult.keys():
                ...
                if p != 'BTC-PERPETUAL':
                    if self.arbmult[p]['arb'] < 1 :
                        self.client.buy(  p, self.maxqty * 2.5 * 5 * 1 / len(self.futures), mid * 1.02, 'false' )
                    else:
